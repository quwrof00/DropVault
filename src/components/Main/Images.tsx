...
export default function Images({ roomId }: ImagesProps) {
 ...
 // Delete file and potential thumbnail
 const paths = [`${prefix}/${fileName}`, `${prefix}/thumb_${fileName}`]
 ...
 // 1️⃣ Delete from storage (including thumbnails)
 const pathsToDelete = names.flatMap(n => {
  const entry = files[n];
  const prefix = entry?.pathPrefix || prefixes.primary;
  return [`${prefix}/${n}`, `${prefix}/thumb_${n}`];
 });
 ...
 if (oldEntry.thumbnailUrl) {
    const oldThumbPath = `${prefix}/thumb_${oldName}`
    const newThumbPath = `${prefix}/thumb_${newName}`
    await supabase.storage.from(BUCKET).move(oldThumbPath, newThumbPath).catch(() => {})
 }
 ...
 {/* Upload Area */}
 <div
 className={`relative border-2 border-dashed rounded-xl p-4 sm:p-8 ${dragOver
 ...
 className={`group rounded-xl shadow-lg overflow-hidden border cursor-pointer
 ${isSelected(name)
 ...
 <input
 type="text"
 className="w-full p-2 rounded-lg border border-gray-600 bg-gray-800 text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
 value={newFileName}
 ...
 disabled={isRenaming}
 className={`flex-1 bg-blue-600 text-white py-2 px-3 rounded-lg hover:bg-blue-700 text-sm font-medium flex items-center justify-center gap-2 ${isRenaming ?"opacity-50 cursor-not-allowed" :""}`}
 >
 ...
 disabled={isRenaming}
 className="flex-1 bg-gray-600 text-gray-300 py-2 px-3 rounded-lg hover:bg-gray-500 text-sm font-medium disabled:opacity-50"
 >
 Cancel
 </button>
 </div>
 ...
 onClick={(e) => e.stopPropagation()}
 className="flex-1 text-center bg-blue-600/20 text-blue-400 py-2 px-3 rounded-lg hover:bg-blue-600/30 text-sm font-medium"
 >
 Preview
 </a>
 </div>
